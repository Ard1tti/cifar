...
def weight_variable(shape, stdev=0.01, wd=0.00):
...
def bias_variable(shape, const=0.02):
...
def residual_layer(x, shape, name='residual'):  
    ...
    with tf.variable_scope('conv2') as scope:
        kernel = weight_variable(shape=shape)
        biases = bias_variable(shape=shape[3])
        temp = tf.nn.conv2d(x, kernel, [1,1,1,1], padding='SAME')
        conv2 = tf.add(temp, biases, name=scope.name)        
    
    return tf.add(conv1, conv2, name=name)
...
def inference(images, keep_prob):
    ...
    with tf.variable_scope('softmax_linear') as scope:
        ...
        weights = weight_variable([256, LABEL_NUM], wd=0.04)
