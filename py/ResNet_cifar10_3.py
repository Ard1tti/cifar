...
def residual_layer(x, shape, name='residual'):
    ...
    shape1=[shape[0],shape[0],shape[3],shape[2]]
    shape2=[shape[1],shape[1],shape[2],shape[2]]
    shape3=[shape[0],shape[0],shape[2],shape[3]]
    
    with tf.variable_scope('conv1') as scope:
        conv1 = conv2d_layer(x, shape1, name=scope.name)
    
    with tf.variable_scope('conv2') as scope:
        conv2 = conv2d_layer(conv1, shape2, name=scope.name)
        
    with tf.variable_scope('linear') as scope:
        kernel = weight_variable(shape=shape3)
        biases = bias_variable(shape=shape[3])
        temp = tf.nn.conv2d(conv2, kernel, [1,1,1,1], padding='SAME')
...
def inference(images, keep_prop, is_training=False, reuse=True):
    ...
    with tf.variable_scope('res1') as scope:
        res1 = residual_layer(norm1, [1, 3, 32, 64], name=scope.name)
    ...
    with tf.variable_scope('res2_1') as scope:
        res2_1 = residual_layer(norm2, [1, 3, 64, 128], name=scope.name)
    with tf.variable_scope('res2') as scope:
        res2 = residual_layer(res2_1, [1, 3, 64, 128], name=scope.name)
    ...
    with tf.variable_scope('res3_1') as scope:
        res3_1 = residual_layer(norm3, [1, 3, 128, 256], name=scope.name)
    with tf.variable_scope('res3') as scope:
        res3 = residual_layer(res3_1, [1, 3, 128, 256], name=scope.name)
