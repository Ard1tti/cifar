...
def inference(images, keep_prop):
    ...
    with tf.variable_scope('avg_pool') as scope:
        avg_pool = tf.nn.avg_pool(res2, ksize=[1,16,16,1], strides=[1,1,1,1],
                                  padding='VALID', name=scope.name)
        
    with tf.variable_scope('fc') as scope:
        reshaped = tf.reshape(avg_pool, [-1, 128])
        weights = weight_variable([128,500], wd=0.04)
        biases = bias_variable([500])
        fc = tf.nn.relu(tf.add(tf.matmul(reshaped,weights), biases), name=scope.name)

    with tf.variable_scope('softmax_linear') as scope:
        
        weights = weight_variable([500, LABEL_NUM], wd=0.04)
