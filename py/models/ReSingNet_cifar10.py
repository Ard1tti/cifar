...
def weight_variable(shape, stdev=0.05, wd=0.04):
...
def bias_variable(shape, const=0.0):
...
def residual_layer(x, shape, name='residual'):
    ...
    return tf.add(conv1, x, name=scope.name)
    
def inference(images, keep_prop):
    with tf.variable_scope('conv1') as scope:
        conv1 = conv2d_layer(images, [5, 5, 3, 64], name=scope.name)
    
    with tf.variable_scope('res1_1') as scope:
        res1_1 = residual_layer(conv1, [3, 3, 64, 64], name=scope.name)
    with tf.variable_scope('res1') as scope:
        res1 = residual_layer(res1_1, [3, 3, 64, 64], name=scope.name)

    with tf.variable_scope('conv2') as scope:
        conv2 = conv2d_layer(res1, [3, 3, 64, 128], stride=2, name=scope.name)

    with tf.variable_scope('res2_1') as scope:
        res2_1 = residual_layer(conv2, [3, 3, 128, 128], name=scope.name)
    with tf.variable_scope('res2') as scope:
        res2 = residual_layer(res2_1, [3, 3, 128, 128], name=scope.name)

    with tf.variable_scope('conv3') as scope:
        conv3 = conv2d_layer(res2, [3,3,128,256], stride=2, name=scope.name)
    
    with tf.variable_scope('res3_1') as scope:
        res3_1 = residual_layer(conv3, [3, 3, 256, 256], name=scope.name)
    with tf.variable_scope('res3') as scope:
        res3 = residual_layer(res3_1, [3, 3, 256, 256], name=scope.name)
